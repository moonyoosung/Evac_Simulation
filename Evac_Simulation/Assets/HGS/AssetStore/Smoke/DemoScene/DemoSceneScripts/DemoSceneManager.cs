...
public class DemoSceneManager : MonoBehaviour {
    ...
    public GameObject[] _particleEffects; //The list of all particle effects the scene can show

    private int _effectIndex = 0;

    void Start()
    {
        foreach(GameObject system in _particleEffects) //Disable every particlesystem
        {
    ...
    {
        _particleEffects[_effectIndex].SetActive(false); //Deactivate current particle effect
        _effectIndex++;
        ...
        _particleEffects[_effectIndex].SetActive(true); //Activate next particle effect
    }
